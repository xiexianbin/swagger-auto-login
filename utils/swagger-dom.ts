import type { AuthMethod } from './storage';
...
export const swaggerDom = {
   ...
   */
  async fillAuthModal(authMethods: AuthMethod[]): Promise<void> {
    ...
    for (const method of authMethods) {
      switch (method.authType) {
        case 'basic':
          await this.fillBasicAuth(method);
          break;
        case 'bearer':
          await this.fillBearerAuth(method);
          break;
        case 'apiKey':
          await this.fillApiKeyAuth(method);
          break;
        case 'oauth2':
          await this.fillOAuth2(method);
          break;
      }
    }
  ...
  async fillBasicAuth(method: AuthMethod): Promise<void> {
    const usernameInput = document.querySelector<HTMLInputElement>(
      'input[name="username"], input[placeholder*="username" i], section[data-name*="basic" i] input[type="text"]'
    );
    const passwordInput = document.querySelector<HTMLInputElement>(
      'input[name="password"], input[placeholder*="password" i], section[data-name*="basic" i] input[type="password"]'
    );
  ...
  async fillBearerAuth(method: AuthMethod): Promise<void> {
  ...
  async fillApiKeyAuth(method: AuthMethod): Promise<void> {
    const apiKeyInput = document.querySelector<HTMLInputElement>(
      `input[name="${method.apiKeyName}"], input[placeholder*="api" i], section[data-name*="api" i] input[type="text"]`
    );
  ...
  async fillOAuth2(method: AuthMethod): Promise<void> {
